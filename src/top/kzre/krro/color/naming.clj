...
(def ^:private named-colors
  {"aliceblue"            [0.941 0.973 0.988]
   ...
   "mediumturquoise"      [0.282 0.820 0.8  ]
...
(defn lookup-name
  "根据颜色名称字符串返回 RGB 向量，忽略大小写。"
  [name]
  (get named-colors (str/lower-case name)))

(defn nearest-name
  "返回与给定 RGB 颜色最接近的 W3C 颜色名。"
  [rgb-color]
  (let [best (apply min-key
                    (fn [[_ named-rgb]]
                      (dist/delta-e2000-rgb rgb-color named-rgb))
                    named-colors)]
    (first best)))